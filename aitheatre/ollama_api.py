...
# Начало сообщения с найденным — для тех моделей, что не держат протокол
# инструментов и получают результат обычным текстом (см. ниже). Строка вынесена
# сюда не ради красоты: по ней же приложение потом узнаёт свой собственный ответ
# инструмента, когда считает, из чего состоял хвост хода (см. show.added_kinds)
SEARCH_RESULT_PREFIX = "Результаты поиска по запросу «"
        ...
        for tc in tool_calls:
            ...
            if func_name == "search_web" and search_count < max_searches:
                ...
                if tc.get("textual"):
                    ...
                    messages.append({
                        "role": "user",
                        "content": f"{SEARCH_RESULT_PREFIX}{query}»:\n\n{result}",
                        "name": "system"
                    })
                else:
                    messages.append({
