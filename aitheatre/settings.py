...
# ── ПОИСК В ИНТЕРНЕТЕ ───────────────────────────────────────────────────────
#
# Ищет всегда приложение, а не модель: и местной, и облачной мы отдаём один и
# тот же инструмент search_web, а по сети ходит наш код (DuckDuckGo). Модель
# может только попросить. Поэтому все числа поиска собраны в одном месте, а не
# разбросаны по коду: сколько раз ЗАСТАВЛЯЕМ искать, сколько разрешаем и сколько
# результатов приносим по каждому запросу.
#
# Ход без поиска — это тоже ход, и это нормально: у моделей бывает свой ответ.
# Но минимальное число поисков мы требуем до первого слова (см. SEARCH_BEFORE_REPLY),
# а сверх максимума — отказываем словами, а не молчанием (см. MAX_SEARCHES).
ENABLE_SEARCH = True

# Сколько поисков ТРЕБОВАТЬ за ход, прежде чем модель скажет слово.
# 0 — не требовать вовсе: модель ищет, когда сама сочтёт нужным. Молчание
# после поиска этим не лечится — за него отвечает MAX_SEARCH_ATTEMPTS
MIN_SEARCHES = 0

# Сколько поисков РАЗРЕШИТЬ за один ход. Сверх этого предела модели отвечают
# «лимит исчерпан», и она обязана досказать словами: без потолка зацикленная
# модель гоняла бы поисковый круг за кругом, а платил бы за это режиссёр
MAX_SEARCHES = 5

# Сколько результатов поиска приносить по одному запросу. Больше пяти
# провайдер всё равно обрезает, а модели это лишний повод для обстоятельной
# реплики — и лишние входные токены, которые платятся как обычные
SEARCH_MAX_RESULTS = 5

# Сколько раз поправлять модель, если ход кончился не репликой: она ответила,
# не поискав (это при MIN_SEARCHES > 0), или искала, а слов так и не сказала.
# Второе работает и при MIN_SEARCHES = 0: искать уже не надо, надо сказать
...
# Пауза после хода, прежде чем заговорит следующий участник. Раньше это была
# половина секунды прямо в коде хода — и о ней никто не знал: ни в логе, ни
# в настройках. Она нужна странице: реплика уезжает в ленту и меняется статус,
# а браузер должен успеть это показать. 0 — не ждать вовсе
TURN_PAUSE = 0.5
...
# ДАМП спектакля: полная хронология того, что происходило на сцене, — по каждому
# ходу видно, что вошло в запрос, что она попросила, что ей принесли, что она
# сказала и сколько токенов за это заплачено. Раньше это лежало в двух местах:
# размышления моделей — в стенограмме (файл рос от спектакля к спектаклю),
# а снимки запросов — только в памяти и только у последних ходов. Читают это
# не ради истории, а ради разбора последнего спектакля: «почему вот тут вышло
# так», поэтому файл один и в нём только последний спектакль (см. start_dump).
# Обычный markdown, папка в .gitignore
DUMP_FILE = instance_dir() / "damp.md"
...
# в разы больше), помня, что история уходит в каждом запросе и оплачивается
...
# vLLM или SGLang на своей машине. На платном ключе история уходит в КАЖДОМ
...
# ВТОРАЯ ПОЛОВИНА СТОЛА — ОТВЕТ, И У ОБЛАКА ОНА НЕ ЗАНИМАЕТСЯ НИЧЕМ.
# У местной модели размер ответа задан в OPTIONS («num_predict») и уходит
# в Ollama числом: он и реплику ограничивает, и место под неё в окне занимает.
# Облачному участнику ЧУЖОЕ число не подставляется: за размер окна истории
# отвечает CLOUD_NUM_CTX, а за длину ответа — сам вендор. Так и просил
# режиссёр: «на ответ их не надо ограничивать, пускай мыслят, ищут, отвечают
# сколько влезет»; дорого не это, а длинная история, которую гоняют туда-сюда
# в каждом запросе.
# CLOUD_MAX_TOKENS — единственное, чем ответ облака ограничивается, и ноль
# здесь значит НОЛЬ: ни max_tokens в тело запроса (шлюз решает сам), ни места
# под ответ в окне (вся вторая половина стола достаётся истории — это видно
# строкой «Окно говорящего» у реплики). Олламовский num_predict сюда больше
# не подставляется: облачный участник терял на нём часть окна ни за что.
# Ненулевое значение уходит как max_tokens ДАЖЕ при выключенных числах
# характеров (CLOUD_SEND_PARAMS): это поле самой схемы OpenAI, его понимают
# все шлюзы, в отличие от temperature и прочих чисел Ollama.
...
# (у gpt-5-nano это уже видели: «размышления заняли весь бюджет»)
CLOUD_MAX_TOKENS = 0
...
CLOUD_TURN_LIMIT = 180

# Сколько секунд добавляет ходу каждый состоявшийся поиск. Срок выше — на ход
# целиком, а у модели, которая ищет, запросов за ход столько же, сколько
# поисков: каждый круг — новый запрос к шлюзу. Надбавка за поиск и есть ответ
# на «хорошо, даём тебе ещё времени, ты главное не торопись, раз хочешь»
CLOUD_SEARCH_TIME = 60

# ОСТАТОК НА КЛЮЧЕ — ЭТО И ЕСТЬ ЦЕНА ХОДА. Тарифов мы не знаем и знать
# не можем: цены шлюза живут в его каталоге, в рублях за 1 000 000 токенов,
# и меняются без спроса. Зато остаток шлюз отдаёт сам, и разница между «до хода»
# и «после хода» — это не оценка, а фактически списанные за этот ход рубли:
# и размышления, и поиски, и кэш в неё уже входят, потому что списываются
# по факту. У ProxyAPI это GET /proxyapi/balance, и ключу нужно разрешение
# «Запрос баланса» (по умолчанию выключено — тогда шлюз отвечает 403, и цены
# театр просто не показывает). Пустая строка — не спрашивать вовсе
CLOUD_BALANCE_PATH = "/proxyapi/balance"

# Как часто пульт может перечитывать остаток (секунды): остаток — это про
# деньги, и дёргать шлюз на каждый взгляд в сторону незачем. Цену хода этот
# кэш не трогает: перед ходом и после него остаток читается заново
CLOUD_BALANCE_TTL = 20
...
def context_budget(model: str = "") -> tuple:
    ...
    OPTIONS["num_predict"], у облачных — своё число, CLOUD_MAX_TOKENS.
    И ноль у облака значит ноль: ответ не ограничиваем и место под него
    в окне не откладываем вовсе (см. CLOUD_MAX_TOKENS). Исключение — окно,
    которое меньше самого запаса: так бывает при опечатке в настройке,
    и расплачиваться за неё история не должна, иначе участник останется
    без неё вовсе.
    """
    if str(model or "").startswith(CLOUD_MODEL_PREFIX):
        ...
        if window <= 0:                      # 0 — окно не ограничено
            return 0, 0
        # Ноль — это «ответ не ограничиваем»: олламовский предел сюда не
        # подставляется, иначе облачная история резалась бы по числу, которое
        # придумано для местных моделей, ни за что
