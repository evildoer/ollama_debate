...
# Номер запуска театра: время, когда этот процесс поднялся. Страница по нему
# видит, что сервер перезапустили: она держит прежний спектакль и прежнюю
# вёрстку, а всё, что вы выбрали в консоли (например, «начать с пустой сцены»),
# выглядит несделанным. При расхождении страница собирается заново с сервера
# (см. page.noteServerBoot). Число как число — сравнивать его легко и нечем
# ошибиться, а счётчика запусков у процесса нет
...
def main():
    ...
    # Браузер при занятом порте не открываем: по этому адресу отвечает прежний
    # экземпляр, и на экране оказался бы его спектакль — а всё, что вы только что
    # выбрали в консоли, выглядело бы несделанным (см. port_conflict_notice).
    # Это тот случай, из которого родилось «нажал Enter, а на странице прежний
    # спектакль и нет формы»
    if busy:
        print("🌐 Браузер не открываю: по этому адресу отвечает прежний экземпляр.")
        print("   Закройте его (номера процессов в предупреждении выше), а потом")
        print("   обновите страницу здесь.")
    else:
