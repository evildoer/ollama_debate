...
def _turn_notes(answer: dict) -> dict:
    ...
    Числа называются «вводом» и «выводом», а не prompt_tokens и completion_tokens:
...
def journal_ask_start(report: dict, tokens_in_est: int = None, tools: bool = None,
    ...
    видно, что запрос был и сколько в нём было. Без этого у хода, оборванного
...
def journal_ask(report: dict, notes: dict) -> dict:
    ...
    всё, что случилось, — сколько было на ввод, что модель попросила, что ей
...
def messages_tokens(messages: list) -> int:
    ...
    на месте ввода осталась бы дыра, а «сколько вошло в запрос» — первое, что хочется
...
def chat(model: str, messages: list, options: dict = None, tool_choice: str = None,
    ...
    if uses_tools:
        ...
        # инструмент всё равно идёт: модель ищет по своей воле, а правила
    ...
    usage = answer.get("usage") or {}
    if usage:
        # Ввод и вывод — словами: по строке «3431 + 1246» нельзя понять, что
        ...
        print(f"  ☁️  {bare_model_name(model)}: ввод {usage.get('prompt_tokens', '?')} "
